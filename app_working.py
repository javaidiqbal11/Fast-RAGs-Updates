...
def get_existing_collections() -> List[str]:
    ...
    # client = QdrantClient(url="http://qdrant_service:6333")
...
def get_embeddings(embed_method, device: Device):
    ...
    if embed_method == EmbedMethod.bge_small_en:
        ...
        emb = "text-embedding-ada-002" # It's not open-source 
...
### Cases
# Case 1: Normal working with Embedding (bge-small-en), not generating respses if not available in the vectorstore but provoding web link 
# Case 2: Best responses on the Embedding (gte-base), generating responses from web link if not available in the vectorstore
